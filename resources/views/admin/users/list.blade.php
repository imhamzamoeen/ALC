...
@section('actions')
    <div class="d-flex align-items-center py-1">
                ...
                <form id="filter-form" method="get" action="{{ url()->current() }}">
                    ...
                    <div class="px-7 py-5">
                        ...
                        <div class="mb-10">
                            ...
                            <div>
                                <select class="form-select form-select-solid" data-kt-select2="true" name="user_type"
                                    data-placeholder="Select option" data-dropdown-parent="#kt_menu_616d17d195841"
                                    data-allow-clear="true">
                                    <option></option>
                                    @foreach (\Spatie\Permission\Models\Role::where('status', \App\Classes\Enums\StatusEnum::Active)->get() as $role)
                                        <option value="{{ $role->name }}"
                                            @if (request()->get('user_type') == $role->name) selected @endif>
                                            {{ beautify_slug($role->name) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
        ...
        <a href="{{ route(auth()->user()->user_type . '.users.add') }}" class="btn btn-sm btn-primary">Create</a>
    </div>
@endsection
                        ...
                        <tbody class="text-gray-600 fw-bold">
                            @foreach ($users as $user)
                                <tr class="odd">
                                    <td>
                                        <div class="form-check form-check-sm form-check-custom form-check-solid">
                                            <input class="form-check-input" name="ids[]" type="checkbox"
                                                value="{{ $user->id }}" form="bulk-action">
                                        </div>
                                    </td>
                                    <td>
                                        {{ $user->id }}
                                    ...
                                    <td>
                                        <div
                                            class="badge badge-light-{{ \App\Classes\Enums\UserTypesEnum::$USER_TYPE_COLOR[@$user->getRoleNames()[0]] ?? 'primary' }} fw-bolder">
                                            {{ isset($user->getRoleNames()[0]) ? beautify_slug($user->getRoleNames()[0]) : '--' }}
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div
                                            class="badge badge-light-{{ \App\Classes\Enums\StatusEnum::$Status_color[$user->status] }} fw-bolder">
                                            {{ beautify_slug($user->status) }}
                                        </div>
                                    </td>
                                    <td data-order="2021-06-20T21:23:00+05:00">
                                        {{ \Carbon\Carbon::parse($user->created_at)->format('d M Y, h:i A') }}
                                    </td>
                                    @role('marketing')
                                        <td>
                                            <div class="badge badge-light fw-bolder">
                                                {{ beautify_slug($user->pf ?? '--') }}
                                            </div>
                                        </td>

                                        <td>
                                            <div class="badge badge-light fw-bolder">
                                                {{ beautify_slug($user->pn ?? '--') }}
                                            </div>
                                        </td>
                                        <td>
                                            <div class="badge badge-light fw-bolder">
                                                {{ beautify_slug($user->country ?? '--') }}
                                            </div>
                                        </td>
                                    @endrole
                                    @hasanyrole('admin|tc|customersupport')
                                        ...
                                        <td class="text-end">
                                            ...
                                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4"
                                                ...
                                                <div class="menu-item px-3">
                                                    <a href="{{ route(auth()->user()->user_type . '.users.edit', $user->id) }}"
                                                        class="menu-link px-3">Edit</a>
                                                </div>
                                                <div class="menu-item px-3">
                                                    <a href="javascript:void(0);" data-name="{{ $user->name }}"
                                                        data-id="{{ $user->id }}"
        ...
        $('.delete-btn').on('click', function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.replace('{{ URL::to('admin/users/destroy') }}/' + $(this).data('id'))
                }
