        ...
        <div class="card-header border-0 pt-6">
            <div class="card-title">
                <div class="d-flex align-items-center position-relative my-1 mr-2   ">
                    ...
                    <input form="filter-form" name="name" type="text" data-kt-user-table-filter="search" class="form-control form-control-solid w-250px ps-14" placeholder="Search {{ $module_title }}" value="{{ request()->get('name') }}">
                </div>
            </div>
        ...
        <div class="card-body pt-0">
            <div id="kt_table_users_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer"><div class="table-responsive"><table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer" id="kt_table_users">
                        ...
                        <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                            ...
                            <th class="min-w-125px sorting" tabindex="0" aria-controls="kt_table_users" rowspan="1" colspan="1" aria-label="User: activate to sort column ascending" style="width: 100.5px;">
                                Title
                            </th>
                            <th class="min-w-125px sorting" tabindex="0" aria-controls="kt_table_users" rowspan="1" colspan="1" aria-label="User Type: activate to sort column ascending" style="width: 270.25px;">
                                Assigned To
                            </th>
                            ...
                            <tr class="odd">
                                ...
                                <td class="d-flex align-items-center">
                                    <div class="d-flex flex-column">
                                        <a href="{{ route(auth()->user()->user_type.'.'.$module_slug.'.view', $row->id) }}" class="text-gray-800 text-hover-primary mb-1">{{ $row->name ?? '--' }}</a>
                                    </div>
                                </td>
                                <td>
                                    @foreach($row->roles->pluck('name') as $role)
                                        <a href="#" class="badge badge-light-{{ \App\Classes\Enums\UserTypesEnum::$USER_TYPE_COLOR[$role] ?? 'info' }} fs-7 m-1">{{ beautify_slug($role) }}</a>
                                    @endforeach
                                </td>
                                <td>
                                    <div class="badge badge-light-{{ \App\Classes\Enums\StatusEnum::$Status_color[$row->status] }} fw-bolder">
                                ...
                                <td class="text-end">
                                    ...
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                        {{--<div class="menu-item px-3">
                                            <a href="{{ route(auth()->user()->user_type.'.'.$module_slug.'.edit', $row->id) }}" class="menu-link px-3">Edit</a>
                                        </div>--}}
                                        <div class="menu-item px-3">
                                            <a href="javascript:void(0);" data-name="{{ $row->name }}" data-id="{{ $row->id }}" class="menu-link px-3 delete-btn">Delete</a>
                                        </div>
                                    </div>
                                </td>
                            </tr>
...
@push('after-script')
    <script>
        $('.delete-btn').on('click', function () {
            Swal.fire({
                text: 'Are you sure you want to delete "'+ $(this).data('name') +'" role?',
                icon: 'warning',
