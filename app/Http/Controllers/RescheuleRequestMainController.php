...
use App\Http\Requests\Reschedule\ApproveReschedulRequest;
use App\Http\Requests\Reschedule\DeclineReschedulRequest;
...
class RescheuleRequestMainController extends Controller
{
    public function ApproveRescheduleRequest(ApproveReschedulRequest $request)
    {
        try {
            ...
            if ($request->filled('teacher_id')) {
                ...
                if ($Reschedule->Requestable_id == $request->teacher_id && $Reschedule->Requestable_type == "App\Models\User") {
                    ...
                    $weeklyclass_update = $old_weekly_class->update([
                        'class_time' => Carbon::parse($Reschedule->reschedule_date)->addMinutes($Reschedule->reschedule_slot_id * 30),   //update the class time 
                    ...
                    // as teacher is approving so student's parent and teacher co ordinator should get mail
                    $emails = [];
                    ...
                    if ($weeklyclass_update > 0 && $notification_update > 0) {
                        //both updates worked perfectily
                        DB::commit();
                        return JsonResponseService::JsonSuccess("Request For Reschedule Approved Successfully", $request->reschedule_class_id);
                    } else {
                        DB::rollback();
                ...
                //student is doing 
                if ($Reschedule->Requestable_id == $request->student_id && $Reschedule->Requestable_type == "App\Models\Student") {
                    ...
                    if ($weeklyclass_update > 0 && $notification_update > 0) {
                        ...
                        // as student is is approving so student's parent and teacher co ordinator and teacher  should get mail
                        $emails = [];
                        // $emails[]=User::find($request->teacher_id);   //find teacher co ordinator
                        $emails[] = ['email' => Student::find($details['student_id'])->user()->value('email'), 'user' => 'parent'];  //parent email address
                        $emails[] = ['email' => User::find($details['teacher_id'])->Teacher_Coordinator()->value('email'), 'user' => 'coordinator'];  //parent email address
                        ...
                        $details = array_merge(['email' => $emails], $details);
                        dispatch(new SendRescheduleMailJob($details));          // send mail to student that teacher has asked for class reschedule
                        // send mail to student that teacher has asked for class reschedule
    ...
    public function DeclineRescheduleRequest(DeclineReschedulRequest $request)
    {
        try {
            // now we are getting appropriatea record 
